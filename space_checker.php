...
// If file, checks whether there is any leading spaces before opening PHP tag or trailing spaces after closing PHP tag
  function check($path ) {
		if (!is_dir($path )) {
//			$contents = file_get_contents($path);
      $fh = fopen($path, 'r');
      $contents = @fread($fh, filesize($path)-1);
      fclose($fh);

			if (preg_match(PRE, $contents))
        echo $path. " — contains leading spaces \n";
			if (preg_match(POST, $contents))
